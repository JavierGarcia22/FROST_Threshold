// PC Coordinator FROST DKG - END-TO-END ENCRYPTED SHARES
...
int main(void) {
    printf("=== FROST DKG with End-to-End Encrypted Shares ===\n");
    ...
    for (int i = 0; i < N; i++) {
        if (participants[i].type == COMM_TYPE_LOCAL_FILE) {
            ...
            for (int j = 0; j < N; j++) {
                ...
                secp256k1_pubkey c1;
                if (!ecc_elgamal_encrypt_value(ctx, receiver_pubkey, local_shares[i][j].value,
                                               &c1, encrypted_local_shares[i][j].c2, false)) {
